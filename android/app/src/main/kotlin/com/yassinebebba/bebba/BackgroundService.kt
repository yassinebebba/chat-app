...
class BackgroundService : Service(), LifecycleDetector.Listener {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        intent?.getLongExtra(BackgroundService.Companion.KEY_CALLBACK_RAW_HANDLE, -1)?.let { callbackRawHandle ->
    ...
    private fun getCallbackRawHandle(): Long? {
        val prefs = getSharedPreferences(BackgroundService.Companion.SHARED_PREFERENCES_NAME, Context.MODE_PRIVATE)
        val callbackRawHandle = prefs.getLong(BackgroundService.Companion.KEY_CALLBACK_RAW_HANDLE, -1)
        return if (callbackRawHandle != -1L) callbackRawHandle else null
    }

    private fun setCallbackRawHandle(handle: Long) {
        val prefs = getSharedPreferences(BackgroundService.Companion.SHARED_PREFERENCES_NAME, Context.MODE_PRIVATE)
        prefs.edit().putLong(BackgroundService.Companion.KEY_CALLBACK_RAW_HANDLE, handle).apply()
    }
    ...
    companion object {
        ...
        fun startService(context: Context, callbackRawHandle: Long) {
            val intent = Intent(context, BackgroundService::class.java).apply {
                putExtra(BackgroundService.Companion.KEY_CALLBACK_RAW_HANDLE, callbackRawHandle)
            }
